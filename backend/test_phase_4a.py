...
# Upload file
file_path = os.path.join(os.path.dirname(__file__), "../docs/test-data-room/01_demo_zoning_certificate_and_scheme_extract.pdf")
file_name = "01_demo_zoning_certificate_and_scheme_extract.pdf"
    ...
    with psycopg.connect(db_url, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            if job["status"] in ["completed", "failed"]:
                ...
                cur.execute('SELECT text FROM "DocumentChunk" WHERE "documentId" = %s LIMIT 1', (document_id,))
                chunk = cur.fetchone()
                if chunk:
