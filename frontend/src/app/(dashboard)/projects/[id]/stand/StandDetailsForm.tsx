...
export function StandDetailsForm({ projectId, initialData, constraints }: { projectId: string, initialData: Partial<import('@prisma/client').Stand>, constraints?: import('@prisma/client').Constraint[] }) {
  ...
  const renderSelect = (name: keyof typeof formData, placeholder: string, options: string[]) => {
    const rawVal = formData[name] as string
    // Auto-correct casing from AI suggestions by finding the matching option case-insensitively
    const matchedOption = options.find(opt => opt.toLowerCase() === rawVal?.toLowerCase())
    const valToUse = matchedOption || rawVal

    return (
      <Select 
        value={valToUse || undefined} 
